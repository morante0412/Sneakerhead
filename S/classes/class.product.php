...
public function new_product_type($tname){
	...
	public function new_product($pname,$desc,$price,$type){
		...
		$stmt1 = $this->conn->prepare("INSERT INTO tbl_product(prod_name, prod_description, prod_date_added, prod_time_added, prod_status, type_id) VALUES (?,?,?,?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row)
			{
			...
			// Insert $price into a different table, with prod_id referencing the newly inserted product in tbl_product
			$stmt2 = $this->conn->prepare("INSERT INTO tbl_product_pricing(prod_id, prod_retail_price) VALUES (?,?)");
...
public function update_product_img($fname,$id){
	...
	function get_prod_price($id){
		$sql="SELECT prod_retail_price FROM tbl_product_pricing WHERE prod_id = :id";	
		$q = $this->conn->prepare($sql);
		$q->execute(['id' => $id]);
